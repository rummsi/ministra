	...
	if ( OPERATORS_PLS_AUTOREFRESH === 1 ) { // check synchronization enabled
		...
		if ( typeof OPERATORS_PLS_URL === 'string' && validateUrl(OPERATORS_PLS_URL) ) { // validate list url
			...
			client.onerror = function () {
				// console.log('ajax error happened! abort.');
				callback();
			};
			try {
				client.send(null);
			} catch ( e ) {echo('catched: ' + e);}
			// abort after some time (20s)
			timeout = setTimeout(function () {
				client.abort();
				callback();
				echo('ajax abort on timeout');
			}, 20000);
		}
	} else {
		callback();
...
function loadEnvironmentVars ( volume ) {
	...
	try {
		...
		[
			'ts_exitType', 'audio_initial_volume', 'autoupdate_cond', 'ssaverDelay', 'autoPowerDownTime', 'ts_time','ts_endType','ts_lag',
