...
if (!empty($_POST)) {
    if (!empty($_POST['login']) && !empty($_POST['password'])) {
        $user = \Ministra\Lib\User::getByLogin($_POST['login']);
        if (!empty($_POST['stb_mac'])) {
            $mac = \Ministra\Lib\S642b6461e59cef199375bfb377c17a39\B05fd1aa4b15191a1638079bdd4bd6b17::o6c94c7b9823303431b00444e69340ade($_POST['stb_mac']);
            $_POST['stb_mac'] = $mac;
            if (!$mac) {
                $error = \_('Error: Not valid mac address');
            } else {
                $user_by_mac = \Ministra\Lib\User::getByMac($mac);
        ...
        if ($error) {
            ...
            $user_id = \Ministra\Lib\User::createAccount($_POST);
            if ($user_id) {
