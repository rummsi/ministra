...
if (@$_GET['del']) {
    ...
    \Ministra\Lib\S642b6461e59cef199375bfb377c17a39\L18e6d54d6202a6e70c8e428830aa4c89::getInstance()->delete('help_city_info', ['id' => (int) $_GET['id']]);
...
if (!$error) {
    if (@$_GET['save'] && !$error) {
        if (@$_POST['title'] && @$_POST['number']) {
            ...
            \Ministra\Lib\S642b6461e59cef199375bfb377c17a39\L18e6d54d6202a6e70c8e428830aa4c89::getInstance()->insert('help_city_info', ['num' => @$_POST['num'], 'title' => @$_POST['title'], 'number' => @$_POST['number']]);
            \header('Location: help_city_info.php');
            exit;
        }
    ...
    if (@$_GET['update'] && !$error) {
        if (@$_POST['title'] && @$_POST['number']) {
            ...
            \Ministra\Lib\S642b6461e59cef199375bfb377c17a39\L18e6d54d6202a6e70c8e428830aa4c89::getInstance()->update('help_city_info', ['title' => $_POST['title'], 'num' => $_POST['num'], 'number' => $_POST['number']], ['id' => (int) @$_GET['id']]);
            \header('Location: help_city_info.php');
            exit;
        }
        $error = \_('Error: all fields are required');
...
?>
    ...
    <tr>
        <td>
            <?php 
$all_info = \Ministra\Lib\S642b6461e59cef199375bfb377c17a39\L18e6d54d6202a6e70c8e428830aa4c89::getInstance()->from('help_city_info')->orderby('num')->get();
...
echo '</table></center>';
if (@$_GET['edit']) {
    $arr = \Ministra\Lib\S642b6461e59cef199375bfb377c17a39\L18e6d54d6202a6e70c8e428830aa4c89::getInstance()->from('help_city_info')->where(['id' => (int) $_GET['id']])->get()->first();
    if (!empty($arr)) {
        $title = $arr['title'];
        $number = $arr['number'];
        $num = $arr['num'];
...
?>"
                                                                             onclick="document.location='help_city_info.php'">
                                    </td>
                                </tr>
                            </table>
                        </form>
