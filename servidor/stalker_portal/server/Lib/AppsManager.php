...
class AppsManager
{
    ...
    {
        $db_apps = \Ministra\Lib\S642b6461e59cef199375bfb377c17a39\L18e6d54d6202a6e70c8e428830aa4c89::getInstance()->from('apps')->orderby('id')->get()->all();
        $lang = $this->lang;
        $apps = \array_map(function ($app) use($without_fetch, $lang) {
            if (!$without_fetch) {
                try {
                    $repo = new \Ministra\Lib\GitHub($app['url']);
                    $info = $repo->getFileContent('package.json');
                } catch (\Ministra\Lib\GitHubException $e) {
                }
                $app['name'] = isset($info['name']) ? $info['name'] : $app['name'];
                $app['alias'] = empty($app['alias']) && isset($info['name']) ? \Ministra\Lib\AppsManager::safeFilename($info['name']) : $app['alias'];
            ...
            if ($app['current_version']) {
                $appsPath = \realpath(PROJECT_PATH . '/../../' . \Ministra\Lib\S642b6461e59cef199375bfb377c17a39\a777f7659bfaad9ba0acb83e0c546a50::getSafe('apps_path', 'stalker_apps/') . $app['alias'] . '/' . $app['current_version']);
                $app['installed'] = !empty($app['alias']) && \is_dir($appsPath);
            } else {
                $app['installed'] = false;
            }
            ...
            $app['app_url'] = 'http' . (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443 ? 's' : '') . '://' . $_SERVER['HTTP_HOST'] . '/' . \Ministra\Lib\S642b6461e59cef199375bfb377c17a39\a777f7659bfaad9ba0acb83e0c546a50::getSafe('apps_path', 'stalker_apps/') . $app['alias'] . '/' . $app['current_version'];
            return $app;
        }, $db_apps);
        return $apps;
    }
    ...
    {
        $app = \Ministra\Lib\S642b6461e59cef199375bfb377c17a39\L18e6d54d6202a6e70c8e428830aa4c89::getInstance()->from('apps')->where(['id' => $app_id])->get()->first();
        if (empty($app)) {
            return false;
        }
        if ($app['current_version']) {
            $app['installed'] = \is_dir(\realpath(PROJECT_PATH . '/../../' . \Ministra\Lib\S642b6461e59cef199375bfb377c17a39\a777f7659bfaad9ba0acb83e0c546a50::getSafe('apps_path', 'stalker_apps/') . $app['alias'] . '/' . $app['current_version']));
        } else {
            $app['installed'] = false;
        }
        $app['app_url'] = 'http' . (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443 ? 's' : '') . '://' . $_SERVER['HTTP_HOST'] . '/' . \Ministra\Lib\S642b6461e59cef199375bfb377c17a39\a777f7659bfaad9ba0acb83e0c546a50::getSafe('apps_path', 'stalker_apps/') . $app['alias'] . '/' . $app['current_version'];
    ...
    {
        $app = $original_app = \Ministra\Lib\S642b6461e59cef199375bfb377c17a39\L18e6d54d6202a6e70c8e428830aa4c89::getInstance()->from('apps')->where(['id' => $app_id])->get()->first();
        if (empty($app)) {
            return false;
        }
        $repo = new \Ministra\Lib\GitHub($app['url']);
        ...
        $app['alias'] = empty($info['alias']) ? \Ministra\Lib\AppsManager::safeFilename($info['name']) : $info['alias'];
        $app['available_version'] = isset($info['version']) ? $info['version'] : '';
        ...
        if (!empty($update_data)) {
            \Ministra\Lib\S642b6461e59cef199375bfb377c17a39\L18e6d54d6202a6e70c8e428830aa4c89::getInstance()->update('apps', $update_data, ['id' => $app_id]);
        }
        unset($app['options']);
        if ($app['current_version']) {
            $app['installed'] = \is_dir(\realpath(PROJECT_PATH . '/../../' . \Ministra\Lib\S642b6461e59cef199375bfb377c17a39\a777f7659bfaad9ba0acb83e0c546a50::getSafe('apps_path', 'stalker_apps/') . $app['alias'] . '/' . $app['current_version']));
        ...
        if (\is_array($releases)) {
            $releases = \array_map(function ($release) use($app, $option_values) {
                ...
                $info = $repo->getFileContent('package.json');
                $option_list = isset($info['options']) ? $info['options'] : [];
    ...
    public function deleteApp($app_id, $version = null)
    {
        ...
        $path = \realpath(PROJECT_PATH . '/../../' . \Ministra\Lib\S642b6461e59cef199375bfb377c17a39\a777f7659bfaad9ba0acb83e0c546a50::getSafe('apps_path', 'stalker_apps/') . $app['alias'] . '/' . $version);
        if (\is_dir($path)) {
            self::delTree($path);
        }
        return false;
    ...
    {
        $need_to_update = \Ministra\Lib\S642b6461e59cef199375bfb377c17a39\L18e6d54d6202a6e70c8e428830aa4c89::getInstance()->from('apps')->where(['status' => 1, 'autoupdate' => 1])->get()->all();
    ...
    public function updateApp($app_id, $version = null)
    {
        ...
        \file_put_contents($tmp_file, \fopen($zip_url, 'r', false, \stream_context_create(['http' => ['header' => "User-Agent: stalker_portal\r\n"]])));
        if (empty($app['alias'])) {
            $app['alias'] = self::safeFilename($app['name']);
        }
        $path = PROJECT_PATH . '/../../' . \Ministra\Lib\S642b6461e59cef199375bfb377c17a39\a777f7659bfaad9ba0acb83e0c546a50::getSafe('apps_path', 'stalker_apps/') . $app['alias'];
        \umask(0);
        if (!\is_dir($path)) {
    ...
    public function installApp($app_id)
    {
        ...
        \file_put_contents($tmp_file, \fopen($zip_url, 'r', false, \stream_context_create(['http' => ['header' => "User-Agent: stalker_portal\r\n"]])));
        $path = PROJECT_PATH . '/../../' . \Ministra\Lib\S642b6461e59cef199375bfb377c17a39\a777f7659bfaad9ba0acb83e0c546a50::getSafe('apps_path', 'stalker_apps/') . self::safeFilename($info['name']);
        \umask(0);
        if (!\is_dir($path)) {
            \mkdir($path, 0777, true);
        }
        ...
        \unlink($tmp_file);
        if (!empty($result)) {
            $update_data = ['current_version' => $latest_release['tag_name']];
            if (empty($app['alias'])) {
    ...
    private function getAppLocalization($app_id, $version = null)
    {
        ...
        $path = PROJECT_PATH . '/../../' . \Ministra\Lib\S642b6461e59cef199375bfb377c17a39\a777f7659bfaad9ba0acb83e0c546a50::getSafe('apps_path', 'stalker_apps/') . $app['alias'] . '/' . $version . '/';
        $app_localizations = [];
        if (!\is_dir($path . '/lang/')) {
            return false;
        }
        $scanned_directory = \array_diff(\scandir($path . '/lang/'), ['..', '.']);
