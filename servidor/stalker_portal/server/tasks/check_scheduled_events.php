...
$_SERVER['TARGET'] = 'ADM';
$schedule_events = \Ministra\Lib\S642b6461e59cef199375bfb377c17a39\L18e6d54d6202a6e70c8e428830aa4c89::getInstance()->from('`schedule_events`')->where(['state' => 1, 'date_begin<=' => 'now()'])->where(['date_end>last_run and date_end>' => 'now()', 'date_end' => 0], 'OR ')->get()->all();
$update_data = [];
$event = new \Ministra\Lib\AdminPanelEvents();
...
foreach ($schedule_events as $row) {
    if (!empty($row['schedule'])) {
        ...
        $next_run_interval = $cronTab->getNextRunDate(\null, \null, \true)->getTimestamp() - $five_minute_interval;
        ...
        if ($add_params = \json_decode($row['recipient'], \true)) {
...
if (!empty($update_data)) {
    ...
    \Ministra\Lib\S642b6461e59cef199375bfb377c17a39\L18e6d54d6202a6e70c8e428830aa4c89::getInstance()->update('`schedule_events`', ['state = IF(periodic=1, state, 0), 
            last_run' => $last_run->format('Y-m-d H:i:s')], [" `id` IN ('" . \implode("', '", $update_data) . "') AND 1=" => 1]);
}
